...
class RobotMarker {
  ...
  setLocation(x, y, theta=0) {
    if(this.marker === null) {
      console.log(this.name + ' marker is null ???');
...
function init() {
  ...
  agents_srv.callService(new ROSLIB.ServiceRequest({}), function(result) {
    ...
    for(let a of agents) {
      ...
      let robot_pose_topic = new ROSLIB.Topic({
        ros : ros,
        name : '/'+a.agent_name+'_agent/robot_pose',
      ...
      robot_pose_topic.subscribe(function(msg) {
        if(!added) {
          console.log('Added '+a.agent_name+' pose to map.');
